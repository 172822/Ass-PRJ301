...
<head>
    <meta charset="UTF-8">
    <title>Khu vực con - Quản lý trọ</title>
    <style>
...
<body>
    <div class="app">
        ...
        <div class="main">
            ...
            <div class="content">
                <h1>Quản lý khu vực con</h1>
                <p><a href="${pageContext.request.contextPath}/subarea?action=add" class="btn btn-primary">Thêm khu vực con</a></p>
                <div class="card">
                    <table>
                        <tr><th>ID</th><th>Khu vực</th><th>Tên</th><th>Thao tác</th></tr>
                        <c:forEach items="${subareas}" var="s">
                            <tr>
                                <td>${s.id}</td>
                                <td><c:forEach items="${areas}" var="a"><c:if test="${a.id == s.areaId}">${a.name}</c:if></c:forEach></td>
                                <td>${s.name}</td>
                                <td>
                                    <a href="${pageContext.request.contextPath}/subarea?action=edit&id=${s.id}" class="btn btn-primary btn-small">Sửa</a>
                                    <form action="${pageContext.request.contextPath}/subarea" method="post" style="display:inline;" onsubmit="return confirm('Xóa?');">
