...
<head>
    <meta charset="UTF-8">
    <title>${subarea != null ? 'Sửa' : 'Thêm'} khu vực con</title>
    <style>
...
<body>
    <div class="app">
        ...
        <div class="main">
            ...
            <div class="content">
                <h1>${subarea != null ? 'Sửa' : 'Thêm'} khu vực con</h1>
                <c:if test="${not empty error}"><p class="error">${error}</p></c:if>
                <div class="card">
                    <form action="${pageContext.request.contextPath}/subarea" method="post">
                        <c:if test="${subarea != null}"><input type="hidden" name="id" value="${subarea.id}"></c:if>
                        <div class="form-group">
                            <label>Khu vực</label>
                            <select name="areaId" required>
                                <c:forEach items="${areas}" var="a">
                                    <option value="${a.id}" ${subarea != null && subarea.areaId == a.id ? 'selected' : ''}>${a.name}</option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Tên khu vực con</label>
                            <input type="text" name="name" value="${subarea != null ? subarea.name : ''}" required>
                        </div>
                        <button type="submit" class="btn btn-primary">Lưu</button>
                        <a href="${pageContext.request.contextPath}/subarea" class="btn">Hủy</a>
                    </form>
                </div>
            </div>
