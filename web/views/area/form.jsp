...
<head>
    <meta charset="UTF-8">
    <title>${area != null ? 'Sửa' : 'Thêm'} khu vực - Quản lý trọ</title>
    <style>
...
<body>
    <div class="app">
        ...
        <div class="main">
            ...
            <div class="content">
                <h1>${area != null ? 'Sửa' : 'Thêm'} khu vực</h1>
                <c:if test="${not empty error}"><p class="error">${error}</p></c:if>
                <div class="card">
                    <form action="${pageContext.request.contextPath}/area" method="post">
                        <c:if test="${area != null}"><input type="hidden" name="id" value="${area.id}"></c:if>
                        <div class="form-group">
                            <label>Tên khu vực</label>
                            <input type="text" name="name" value="${area != null ? area.name : ''}" required>
                        </div>
                        <button type="submit" class="btn btn-primary">Lưu</button>
                        <a href="${pageContext.request.contextPath}/area" class="btn">Hủy</a>
                    </form>
                </div>
            </div>
